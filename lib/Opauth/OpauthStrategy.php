...
class OpauthStrategy
{
    ...
    {
        return (method_exists($this, $action) ? $this->{$action}() : $this->{$defaultAction}());
    }
     ...
     * @param string $profilePath Path to a $profile property. Use dot(.) to separate levels.
     *        eg. Path to $profile['a']['b']['c'] would be 'a.b.c'
     * @param string $authPath    Path to $this->auth that is to be set.
     *
     * @return bool
     */
    protected function mapProfile($profile, $profilePath, $authPath)
    {
        $from = explode('.', $profilePath);
